...
public interface AggregateStockDataRepository extends CrudRepository<Stock, Long> {
    ...
    @Query(value = "select new com.spring.code.challenge.stocksdataset.domain.AggregateStockData(MAX(stocks.price) AS maxPrice, MIN(stocks.price) AS minPrice, SUM(stocks.volume) AS sumOfVolume) " +
